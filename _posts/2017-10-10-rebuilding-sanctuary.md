---
layout: post
title: Rebuilding Sanctuary
tag: video games
banner: "2017-10-10-rs-1507672166622.jpeg"
alt: "The Vaul-Tec office. Parked on top of the office is a Vault-Tec van. Neon signage reads: SECURE YOUR VAULT SPACE TODAY!!!"
---

I've spent a lot of time playing Fallout 4. Most of that time has been spent crafting things, such as attempting to construct a [memory circuit]({{ site.baseurl }}{% post_url 2016-12-28-failing-to-bring-tv-to-fallout %}). But by far the largest time-sink has been rebuilding Sanctuary.
<!--more-->

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229210851.jpg"
	alt="The view into Sanctuary. Players of the game will notice the wall is a pretty huge departure."
%}

I did this all on the PS4, with mods that enable more of the in game items in building mode. As should be immediately obvious if you've ever played the PS4 version, this town is massively over the settlement limit size. I managed to do this by finding a series of size-hacks. Originally anything you dropped and then removed would lower the settlement size; however, only a few days into this an update broke that. The second method I found was to remove a wire while in building mode with the desired item already selected. Once the wire is removed, you can place the item. During the build, the town was littered with conduits placed for my convenience. 

The following is a brief tour.

{%
	include
	youtubePlayer.html
	id="aozfEEQd5WY"
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211313.jpg"
	caption="Welcome to town. Check in with security on the bridge first."
	alt="From the bridge into Sanctuary you can see the security office on the end of the bridge, as well as the new diner and hotel."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211345.jpg"
	caption="Start your visit at our charming diner."
	alt="A Diner with giant Neon Signage reading DINER. Motorcycles sit on the roof."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507671988719.jpeg"
	alt="The diners eating area features the front ends of classic cars emerging from the walls above the heads of the diners."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211417.jpg"
	alt="A front view of the diner&apos;s bar."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507671998492.jpeg"
	caption="Just one of the regulars."
	alt="An alien drinks from a cup in the diner&apos;s eating area."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672003946.jpeg"
	caption="The Sanctuary Inn greets tired travelers at the gates with its comfortable rooms."
	alt="On top of the first house on the right a hotel has been built with an all glass facade. It rises two stories above the house itself."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211610.jpg"
	caption="The finest hotel in the wasteland is a hostel."
	alt="A view of one of the hotel rooms. Many beds are crammed in."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211531.jpg"
	caption="Next door is the only Laundromat you'll find in the wasteland."
	alt="The laundromat is inside of the Tower Laundry, a small tower built from stacked train car parts."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672016238.jpeg"
	caption="Get a haircut while you're here in town."
	alt="A hair salon with waiting seats and two cutting stations."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211515.jpg"
	caption="The Barn is a great place to furnish your house, get a health checkup, and purchase and test the latest armaments."
	alt="The Barn is a barn built onto the back of the Diner that is covered in art work. On its roof sits a gun shop named Blood Bath and Beyond that is accessible via a crosswalk from the Laundry tower. The first floor of the barn is a department store, with the second floor containing a medical clinic."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672025707.jpeg"
	caption="Need a new bedroom set or a new outfit?"
	alt="A view of the department store floor inside The Barn."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211713.jpg"
	caption="A root canal from a robot?"
	alt="A Mr. Medic mans the clinic desk."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211753.jpg"
	caption="What you really want is a gun though, isn't it?"
	alt="An angled view of the Blood Bath and Beyond neon signage. Lit arrow signs point to the entrance."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672041696.jpeg"
	caption="Well Blood Bath and Beyond is happy to help."
	alt="A synth mans the sales desk of Blood Bath and Beyond."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672046951.jpeg"
	caption="Need some new armor?"
	alt="The armor display within Blood Bath and Beyond."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211527.jpg"
	caption="Let's continue in."
	alt="A view from the barns entrance down the street. The crosswalk for Blood Bath and Beyond is visible along the top. Those familiar with Sanctuary&apos;s default state will note how tall it feels after this build out."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211850.jpg"
	caption="Make sure you stop in at Sturges Brahmin-Q, Voted No1 in the Commonwealth by, uh, well someone..."
	alt="Front view of Sturge&apos;s Brahmin-Q."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672061192.jpeg"
	alt="A view of Sturge&apos;s Brahmin-Q during a green radiation storm."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229211949.jpg"
	caption="Join the Minute Men and defend the town!"
	alt="The front of the Minute Men recruitment office. A roof deck has been built to support mortars, and a tank is visible overhanging the roof deck. Minute Men flags line the sidewalk."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212034.jpg"
	caption="Or apply for a job with the Institute at their newest above ground office complex."
	alt="The Institute office is an imposing building with an overhanging roofline, and towers at either front corner."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672076592.jpeg"
	caption="Host your next conference!"
	alt="A nice looking conference room consisting of Institue furniture."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672081317.jpeg"
	caption="Education is important."
	alt="An old American style red single room school house."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672085838.jpeg"
	alt="School desks lined up in the school house."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212250.jpg"
	caption="Be sure to checkout your restored pre-war home."
	alt="The characters home has been restored using the games barn parts."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672094675.jpeg"
	alt="New hedges line home. The door is open, previewing the new floors."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672098507.jpeg"
	alt="A view of the front room of the home."
	imageSize="half"
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672102798.jpeg"
	alt="Shauns Room"
	imageSize="half"
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672107623.jpeg"
	caption="The dogs live well in Sanctuary."
	alt="Dogmeat chilling in his dog home inside a larger dog home. Another junkyard dog hangs out with him."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212333.jpg"
	caption="Checkout the Community Center below our high-end condos."
	alt="Vault sections have been placed on the roof of the home across from the survivors. Inside are high end condos."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672115685.jpeg"
	alt="A view of one condo&apos;s living room."
	size="half"
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672118843.jpeg"
	alt="A view of one condo&apos;s bedroom."
	size="half"
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672122896.jpeg"
	caption="The nicest toilet in the wasteland."
	alt="A condo bathroom. Everything is so shiny."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212534.jpg"
	caption="Be sure to checkout NukaCafe."
	alt="A Nuka Cola themed cafe has been built at the corner of the cul de sac. The outside features a smattering of Nuka memorabilia."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672132622.jpeg"
	alt="Side view of the NukaCafe."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672137596.jpeg"
	alt="The inside of the NukaCafe. A Nuka Cola bottle shaped robot mans the counter."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212604.jpg"
	caption="Some of the NukaCafe food is grown up-stairs."
	alt="An interior view of the greenhouse built on top of NukaCafe."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672149354.jpeg"
	alt="A view of a restored home near the cul de sac."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212624.jpg"
	caption="Some folks grown their own food above their homes."
	alt="A home with a greenhouse built on top."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672161811.jpeg"
	alt="An interior view of the greenhouse on top of the home."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672166622.jpeg"
	caption="Make sure you reserve your Vault Space today, in case everything gets nuked again."
	alt="The Vaul-Tec office. Parked on top of the office is a Vault-Tec van. Neon signage reads: SECURE YOUR VAULT SPACE TODAY!!!"
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212655.jpg"
	caption="We have plenty of power."
	alt="The powerplant is a nondescript concrete building."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672175921.jpeg"
	caption="Make sure you check out the Institute Pavilion."
	alt="Entry to the pavilion. Neon Signage for the pavilion is visible, along with a marquee."
%}

{% include
	post-image.html
	image="2017-10-10-rs-1507672180326.jpeg"
	caption="Now Performing: Death of a Caravan"
	alt="A close up of the marquee"
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212759.jpg"
	alt="A view of the pavilion."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212810.jpg"
	caption="Grab a drink during the show."
	alt="The bar at the pavilion."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161229212837.jpg"
	caption="Let the kids play while the show goes on."
	alt="The original playground remains, but has been cleaned up. A new wooden path leads up to the street."
%}

{% include
	post-image.html
	image="2017-10-10-rs-Fallout+4_20161123160221.jpg"
	caption="She's so excited she is standing in a chair."
	alt="A woman stands on a gorilla chair. Because Fallout wouldn&apos;t be Fallout without collision and pathfinding bugs."
%}
